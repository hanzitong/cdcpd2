#include "cdcpd_ros/kinect_sub.h"

#include <cv_bridge/cv_bridge.h>
#include <image_geometry/pinhole_camera_model.h>
#include <sensor_msgs/image_encodings.hpp>

namespace sm = sensor_msgs::msg;

KinectSub::KinectSub(const std::function<void(cv::Mat, cv::Mat, cv::Matx33d)>& _externCallback,
                     const SubscriptionOptions _options)
    : options(_options), externCallback(_externCallback) {
  
  if (!options.node) {
    RCLCPP_ERROR(rclcpp::get_logger("KinectSub"), "Node pointer is null");
    return;
  }

  it = std::make_shared<image_transport::ImageTransport>(options.node);
  
  // Create subscribers
  rgb_sub = std::make_shared<image_transport::SubscriberFilter>();
  rgb_sub->subscribe(options.node.get(), options.rgb_topic, "raw");
  
  depth_sub = std::make_shared<image_transport::SubscriberFilter>();
  depth_sub->subscribe(options.node.get(), options.depth_topic, "raw");
  
  cam_sub = std::make_shared<message_filters::Subscriber<sm::CameraInfo>>(
      options.node, options.cam_topic);

  sync = std::make_shared<message_filters::Synchronizer<SyncPolicy>>(
      SyncPolicy(options.queue_size), *rgb_sub, *depth_sub, *cam_sub);
  
  sync->registerCallback(
      std::bind(&KinectSub::imageCb, this, 
                std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
}

void KinectSub::imageCb(const sm::Image::ConstSharedPtr& rgb_msg, 
                        const sm::Image::ConstSharedPtr& depth_msg,
                        const sm::CameraInfo::ConstSharedPtr& cam_msg) {
  cv_bridge::CvImagePtr cv_rgb_ptr;
  try {
    cv_rgb_ptr = cv_bridge::toCvCopy(rgb_msg, sensor_msgs::image_encodings::RGB8);
  } catch (cv_bridge::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("KinectSub"), "RGB cv_bridge exception: %s", e.what());
    return;
  }

  cv_bridge::CvImagePtr cv_depth_ptr;
  try {
    cv_depth_ptr = cv_bridge::toCvCopy(depth_msg, depth_msg->encoding);
  } catch (cv_bridge::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("KinectSub"), "Depth cv_bridge exception: %s", e.what());
    return;
  }

  if (depth_msg->encoding != sensor_msgs::image_encodings::TYPE_16UC1) {
    RCLCPP_INFO_THROTTLE(rclcpp::get_logger("KinectSub"), *options.node->get_clock(), 10000,
                         "Depth message is not in %s format. Converting.", 
                         sensor_msgs::image_encodings::TYPE_16UC1);
    if (depth_msg->encoding == sensor_msgs::image_encodings::TYPE_32FC1) {
      cv::Mat convertedDepthImg(cv_depth_ptr->image.size(), CV_16UC1);

      const int V = cv_depth_ptr->image.size().height;
      const int U = cv_depth_ptr->image.size().width;

      for (int v = 0; v < V; ++v) {
        for (int u = 0; u < U; ++u) {
          // Convert from meters to millimeters
          float depth_meters = cv_depth_ptr->image.at<float>(v, u);
          convertedDepthImg.at<uint16_t>(v, u) = static_cast<uint16_t>(depth_meters * 1000.0f);
        }
      }

      cv_depth_ptr->encoding = sensor_msgs::image_encodings::TYPE_16UC1;
      cv_depth_ptr->image = convertedDepthImg;
    } else {
      RCLCPP_ERROR_THROTTLE(rclcpp::get_logger("KinectSub"), *options.node->get_clock(), 10000,
                            "Unhandled depth message format %s", depth_msg->encoding.c_str());
      return;
    }
  }

  if (externCallback) {
    image_geometry::PinholeCameraModel cameraModel;
    cameraModel.fromCameraInfo(cam_msg);
    externCallback(cv_rgb_ptr->image, cv_depth_ptr->image, cameraModel.fullIntrinsicMatrix());
  }
}
