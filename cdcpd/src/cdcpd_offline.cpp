...
constexpr auto const LOGNAME = "cdcpd_offline";
...
int main(int argc, char* argv[]) {
  ...
  auto node = std::make_shared<rclcpp::Node>("cdcpd_offline");
  ...
  auto cdcpd = CDCPD(node, tracked_points, template_edges, use_recovery, alpha, beta, lambda, k_spring, zeta,
  ...
  for (int i = 0; i < frame_num; i++) {
    ...
    auto const hsv_mask = getHsvMask(node, rgb);
