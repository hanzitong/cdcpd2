      ...
      const visualization_msgs::msg::MarkerArray&)>& display_fn)
{
  ...
  const std_msgs::msg::ColorRGBA collision_color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(1.0, 0.0, 0.0, 0.5);
  const std_msgs::msg::ColorRGBA free_color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(0.0, 0.0, 0.0, 0.0);
  const std_msgs::msg::ColorRGBA unknown_color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(0.0, 0.0, 0.0, 0.0);
  ...
  visualization_msgs::msg::Marker distance_rep;
  // Populate the header
  distance_rep.header.frame_id = "world";
  // Populate the options
  distance_rep.ns = "estimated_distance_display";
  distance_rep.id = 1;
  distance_rep.type = visualization_msgs::msg::Marker::CUBE_LIST;
  distance_rep.action = visualization_msgs::msg::Marker::ADD;
  distance_rep.lifetime = rclcpp::Duration(std::chrono::duration<double>(0.0));
  ...
  for (double x = 0; x < sdf.GetXSize(); x += step)
  {
    for (double y = 0; y < sdf.GetYSize(); y += step)
    {
      ...
      //for (double z = 0; z <= sdf.GetZSize(); z += step)
      {
        ...
        if (distance >= 0.0)
        {
          const std_msgs::msg::ColorRGBA new_color
        ...
        {
          const std_msgs::msg::ColorRGBA new_color
              = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(1.0, 0.0, 1.0, 1.0);
          distance_rep.colors.push_back(new_color);
        }
      ...
      for (int64_t z_idx = 0; z_idx < sdf.GetNumZCells(); z_idx++)
      {
        ...
        if (discrete_gradient.size() == 3)
        {
          const Eigen::Vector4d gradient_vector(discrete_gradient[0],
                                                ...
                                                discrete_gradient[2],
                                                0.0);
          visualization_msgs::msg::Marker gradient_rep;
          // Populate the header
          ...
          gradient_rep.type = visualization_msgs::msg::Marker::ARROW;
          gradient_rep.action = visualization_msgs::msg::Marker::ADD;
          gradient_rep.lifetime = rclcpp::Duration(std::chrono::duration<double>(0.0));
          ...
          gradient_rep.color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(1.0, 0.5, 0.0, 1.0);
        ...
        if (smooth_gradient.size() == 3)
        {
          ...
          gradient_rep.scale.y = sdf.GetResolution() * 0.125;
          gradient_rep.scale.z = 0.0;
          gradient_rep.color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(0.0, 0.5, 1.0, 1.0);
          markers.markers.push_back(gradient_rep);
        }
        if (autodiff_gradient.size() == 3)
        {
          ...
          gradient_rep.scale.y = sdf.GetResolution() * 0.125;
          gradient_rep.scale.z = 0.0;
          gradient_rep.color = arm_helpers::RGBAColorBuilder<std_msgs::msg::ColorRGBA>::MakeFromFloatColors(0.5, 0.0, 1.0, 1.0);
          markers.markers.push_back(gradient_rep);
        }
      }
    }
