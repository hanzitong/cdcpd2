      ...
      const COLLISION_CELL&, std::vector<uint8_t>&)>& value_serializer) const
{
  ...
  arm_utilities::SerializeEigen<Eigen::Isometry3d>(inverse_origin_transform_,
                                             buffer);
  // Serialize the data
  arm_utilities::SerializeVector<COLLISION_CELL>(
        data_, buffer, arm_utilities::SerializeFixedSizePOD<COLLISION_CELL>);
  ...
  arm_utilities::SerializeFixedSizePOD<COLLISION_CELL>(default_value_, buffer);
  // Serialize the OOB value
  arm_utilities::SerializeFixedSizePOD<COLLISION_CELL>(oob_value_, buffer);
      ...
      const std::vector<uint8_t>&, const uint64_t)>& value_deserializer)
{
  ...
  const std::pair<std::vector<COLLISION_CELL>, uint64_t> data_deserialized
      = arm_utilities::DeserializeVector<COLLISION_CELL>(
  ...
  const std::pair<COLLISION_CELL, uint64_t> default_value_deserialized
      = arm_utilities::DeserializeFixedSizePOD<COLLISION_CELL>(buffer,
                                                               current_position);
...
sdf_tools::msg::CollisionMap CollisionMapGrid::GetMessageRepresentation(
    const CollisionMapGrid& map)
{
  sdf_tools::msg::CollisionMap map_message;
...
visualization_msgs::msg::Marker CollisionMapGrid::ExportForDisplay(
    ...
    const std_msgs::msg::ColorRGBA& unknown_color) const
{
  ...
  display_rep.lifetime = rclcpp::Duration::from_seconds(0.0);
...
visualization_msgs::msg::Marker CollisionMapGrid::ExportSurfacesForDisplay(
    const std_msgs::msg::ColorRGBA& collision_color,
    const std_msgs::msg::ColorRGBA& free_color,
    const std_msgs::msg::ColorRGBA& unknown_color) const
{
